...
SERVER_A_IP="$1"
SERVER_B_IP="$2"
SERVER_IP="" # This will be updated for each test run
...
# --- FIX: HELPER FUNCTIONS (were missing) ---
...
app_hls_loop() {
  # This variable needs to be redefined inside run_bundle before use
...
csv_header() {
  if [[ ! -f "$CSV" ]]; then
    echo "exp,topo,cc,mtu,impair,load,duration_s,ping_avg_ms,ping_mdev_ms,tcp_sum_gbps,tcp_retr,udp_gbps,udp_loss_pct,udp_jitter_ms,cpu_client_pct,app_ttfb_s,app_total_s,app_hls_avg_s,notes" | sudo tee "$CSV" >/dev/null
  fi
  if [[ ! -f "$SWITCH_LOG" ]]; then
    echo "timestamp,experiment,cc_mode,duration_s" | sudo tee "$SWITCH_LOG" >/dev/null
  fi
}
...
run_bundle() {
  ...
  # Select the correct server IP based on the topology for this test run.
  if [[ "$TOPO" == "T1" ]]; then
    SERVER_IP="$SERVER_A_IP" # T1 = intra-AZ
  elif [[ "$TOPO" == "T2" ]]; then
    SERVER_IP="$SERVER_B_IP" # T2 = cross-AZ
  else
    SERVER_IP="$SERVER_A_IP" # Default to intra-AZ
  fi
  echo "[*] $EXP: topo=$TOPO (target server: $SERVER_IP) cc=$CC mtu=$MTU impair=$IMP load=$LOAD"

  # Only set CC manually if it's NOT a hybrid test.
  if [[ "$CC" != "hybrid" ]]; then
    cc_set "$CC"
  else
  ...
  # Run application-layer tests for the H4 experiment.
  if [[ "$EXP" == "H4" ]]; then
...
if [[ -n "${RESULT_S3_URI:-}" ]]; then
  TS=$(date +%Y%m%d-%H%M%S)
  /usr/bin/aws s3 cp "$CSV" "${RESULT_S3_URI%/}/net-experiments-client-$TS.csv"
  /usr/bin/aws s3 cp "$SWITCH_LOG" "${RESULT_S3_URI%/}/cc_switch_events_client.csv" # This will be empty, which is expected.
fi